class Length
  include Comparable # if you include Comparable you don't need <, <=, ==, >, >= methods
  ...
  def ==(other)
    case unit
    when :km  then (value - other.as_kilometers.value).abs < 0.001
    when :mi  then (value - other.as_miles.value).abs < 0.001
    when :nmi then (value - other.as_nautical_miles.value).abs < 0.001
    end
  end
